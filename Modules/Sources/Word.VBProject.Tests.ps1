...
InModuleScope 'Word.VBProject' {
  BeforeAll {
    ...
    function Get-FixtureWorkbook {
      ...
      [OutputType([string])]
      param ()
      return $PSScriptRoot | Join-Path -ChildPath '..\..\Tests\Bin\Document.docm'
    }
    function Get-FixtureSource {
      [CmdletBinding()]
      [OutputType([string])]
      param ()
      return $PSScriptRoot | Join-Path -ChildPath '..\..\Tests\Modules\Document\VBProject.json'
    }
    ...
    New-WordFile -Path $bin -FileFormat wdFormatXMLDocumentMacroEnabled -Force
    try {
      Import-WordVBProject -Path $bin -Source (Get-FixtureSource)
    } catch {
  ...
  AfterAll {
    ...
    Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination
  }
  Describe 'Export-WordVBProject' {
    ...
    Context 'ParameterSetName' {
      It 'exports VBProject by FilePath as JSON' {
        $item = Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination
      ...
      It 'exports VBProject by Path with ValueFromPipeline' {
        $item = (Get-FixtureWorkbook) | Export-WordVBProject -Destination $destination
        $exported = Get-Content -LiteralPath $destination -Encoding UTF8 | ConvertFrom-Json
        $item.FullName | Should -Be ([Path]::GetFullPath($destination))
        Test-Path -LiteralPath $componentRoot -PathType Container | Should -BeTrue
        @($exported.VBComponents).Count | Should -BeGreaterThan 0
      ...
      It 'exports VBProject by Path with ValueFromPipelineByPropertyName' {
        $item = [PSCustomObject]@{ Path = (Get-FixtureWorkbook) } | Export-WordVBProject -Destination $destination
        $exported = Get-Content -LiteralPath $destination -Encoding UTF8 | ConvertFrom-Json
        $item.FullName | Should -Be ([Path]::GetFullPath($destination))
        Test-Path -LiteralPath $componentRoot -PathType Container | Should -BeTrue
        @($exported.VBComponents).Count | Should -BeGreaterThan 0
      ...
      It 'exports VBProject components to the specified ComponentRoot' {
        ...
        try {
          $item = Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination -ComponentRoot $customRoot
          $exported = Get-Content -LiteralPath $destination -Encoding UTF8 | ConvertFrom-Json
          $item.FullName | Should -Be ([Path]::GetFullPath($destination))
    ...
    Context 'SupportsShouldProcess' {
      It 'does not create outputs when WhatIf is specified' {
        Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination -Force -WhatIf
      ...
      It 'overwrites an existing read-only destination when Force is specified' {
        ...
        { Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination } | Should -Throw
        Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination -Force -Confirm
      ...
      It 'throws and keeps the existing destination content unchanged when NoClobber is specified' {
        ...
        { Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination -NoClobber } | Should -Throw
        { Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination -Force -NoClobber } | Should -Throw
    ...
    Context 'Edge cases' {
      It 'throws when the destination is an existing directory' {
        ...
        { Export-WordVBProject -Path (Get-FixtureWorkbook) -Destination $destination -Force } | Should -Throw
