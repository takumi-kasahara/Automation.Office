...
function New-OneNoteNotebook {
  ...
  .PARAMETER Path
    The file system path where the new notebook directory will be created.
...
function New-OneNoteSection {
  ...
  .PARAMETER Path
    The file system path where the new section file will be created.
...
function New-OneNoteSectionGroup {
  ...
  .PARAMETER Path
    The file system path of the new section group directory.
...
function New-OneNotePage {
  ...
  .PARAMETER SectionId
    The object ID of the section in which the new page will be created.
...
function Get-OneNoteHierarchy {
  ...
  .PARAMETER Id
    The OneNote object ID of the starting node.
  ...
  .PARAMETER Force
    Overwrites read-only output files when present.
...
function Get-OneNotePageContent {
  ...
  .PARAMETER Id
    The OneNote object ID of the page to retrieve.

  .PARAMETER PageInfo
    Specifies what additional information to include in the page content.
...
function Export-OneNoteHierarchy {
  ...
  .DESCRIPTION
    ...
    The output file is named `<Id>.hierarchy.xml` by default, or `<name>.hierarchy.xml` when -UseName is specified.
    File timestamps (CreationTime, LastWriteTime) are set from the hierarchy XML attributes when available.

  .PARAMETER Id
    The OneNote object ID of the element whose hierarchy will be exported.

  .PARAMETER Destination
    The directory path where the hierarchy XML file will be saved.
    The directory will be created if it does not exist.

  .PARAMETER HierarchyScope
    The depth of the hierarchy to export.
    Defaults to hsSelf (only the specified object).

  .PARAMETER UseName
    When specified, the output file is named using the OneNote object's display name
...
function Export-OneNotePageContent {
  ...
  .PARAMETER Id
    The OneNote object ID of the page whose content will be exported.

  .PARAMETER Destination
    The directory path where the content XML file will be saved.
    The directory will be created if it does not exist.

  .PARAMETER PageInfo
    Specifies what additional information to include in the exported content.
    Defaults to piBasic.
...
function Export-OneNotePageAsDocument {
  ...
  .DESCRIPTION
    Publishes the specified OneNote page to a file in the specified destination directory using the OneNote COM API Publish method.
    The output file is named `<Id>.<ext>` by default, or `<name>.<ext>` when -UseName is specified.
    The file extension is determined by the -PublishFormat parameter.

  .PARAMETER Id
    The OneNote object ID of the page to export.

  .PARAMETER Destination
    The directory path where the document will be saved.
    The directory will be created if it does not exist.

  .PARAMETER PublishFormat
    The format to use when publishing the page.
...
function Export-OneNoteBinaryObject {
  ...
  .DESCRIPTION
    Retrieves all binary objects (images, attachments) from the specified OneNote page and saves each one as a file in a subdirectory under the destination directory.
  ...
  .PARAMETER Id
    The OneNote object ID of the page from which binary objects will be exported.

  .PARAMETER Destination
    The parent directory path where the binary object subdirectory will be created.
    The directory will be created if it does not exist.
  ...
  .PARAMETER NoClobber
    Throws an error if an output file already exists.

  .PARAMETER UseName
    When specified, the subdirectory is named using the page's display name
    instead of its object ID.
...
function Import-OneNoteHierarchy {
  ...
  .DESCRIPTION
    Reads a hierarchy XML file previously exported by Export-OneNoteHierarchy and applies it to the OneNote application using the UpdateHierarchy COM API method.
...
function Import-OneNotePageContent {
  ...
  .EXAMPLE
    $pageId = New-OneNotePage -SectionId $sectionId
    $xmlFile = Export-OneNotePageContent -Id $pageId -Destination $env:TEMP
  ...
  .EXAMPLE
    Get-ChildItem -Path "$env:TEMP\*.content.xml" | Import-OneNotePageContent -Confirm:$false
